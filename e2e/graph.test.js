...
test.describe('CodeGraph E2E', () => {
  ...
  test('should display and interact with the timeline', async ({ page }) => {
    ...
    const meta = ticks.nth(1).locator('.eyeloss-timeline__tick-meta');
    await expect(meta).toBeVisible();
    await expect(meta).toContainText('Previous commit');

    const tickId = ticks.nth(1).locator('.eyeloss-timeline__tick-id');
